...
includes_provides: Dict[str, List[str]] = {
    "algorithm": ["std::max", "std::min", "std::reverse", "std::sort"],
    "cctype": ["std::isspace"],
    "chrono": ["std::chrono"],
    "cmath": ["std::abs"],
...
def check_std_includes_in_file(file: pathlib.Path) -> Optional[Panel]:
    lines: List[str] = []
    with file.open() as f:
        lines = f.readlines()
    ...
    if len(panel_lines) > 0:
        return Panel.fit(
            "\n".join(panel_lines), title=file.name, border_style="dark_orange3"
        )
...
def files_to_check():
    return [cpp_file for cpp_file in pathlib.Path('.').glob('chapter*/**/*.cpp')]


def main():
    files: List[pathlib.Path] = files_to_check()

    for cpp_file in files:
        pane = check_std_includes_in_file(cpp_file)
        if pane:
            print(pane)
